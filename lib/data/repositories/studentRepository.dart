...
class StudentRepository {
  ...
  Future<List<StudentByExam>> getStudentsByExamAllocation(
      {required int classSectionId, required int examId, required subjectId}) async {
    try {
      final result = await Api.get(
          url: Api.getStudentsByExamAllocation,
          useAuthToken: true,
          queryParameters: {"exam_id":examId,"class_section_id": classSectionId,"subject_id":subjectId});

      return (result['data'] as List)
          .map((e) => StudentByExam.fromJson(Map.from(e)))
          .toList();
    } catch (e) {
      throw ApiException(e.toString());
    }
  }
  Future<List<StudentByRoutine>> getStudentsByRoutineResult(
      {required int classSectionId, required subjectId}) async {
    try {
      final result = await Api.get(
          url: Api.getStudentsByRoutine,
          useAuthToken: true,
          queryParameters: {"class_section_id": classSectionId,"subject_id":subjectId});

      return (result['data'] as List)
          .map((e) => StudentByRoutine.fromJson(Map.from(e)))
          .toList();
    } catch (e) {
      throw ApiException(e.toString());
    }
...
//This method is used to update subject marks by subjectId
  Future<Map<String, dynamic>> updateSubjectRoutineMarksBySubjectId(
      {
      required int subjectId,
      required Map<String, dynamic> bodyParameter}) async {
    try {
      ...
      final result = await Api.post(
          body: bodyParameter,
          url: Api.submitRoutineMarksBySubjectId,
          useAuthToken: true,
          queryParameters: queryParameters);
  ...
  Future<List<StudentResult>> fetchStudentCompletedExamListWithResult({
    ...
    Future<List<Assessment>> getAssessment(
      {required int studentId}) async {
    try {
      final result = await Api.get(
          url: Api.getAssessment,
          useAuthToken: true,
          queryParameters: {"student_id": studentId});

       return ((result['data'] ?? []) as List)
          .map((result) => Assessment.fromJson(Map.from(result)))
          .toList();
    } catch (e) {
      throw ApiException(e.toString());
    }
